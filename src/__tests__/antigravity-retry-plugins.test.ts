...
describe('Antigravity client retry identities', () => {
  ...
  it('gives each Pi logical invocation a new ID without changing other providers', async () => {
    type Hook = (event: { payload: Record<string, unknown> }, context: { model?: { provider: string } }) => Record<string, unknown> | undefined
    ...
    plugin.default({ on: (name, handler) => { expect(name).toBe('before_provider_request'); hook = handler } })
