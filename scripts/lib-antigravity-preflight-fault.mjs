...
export async function preflightFault(root, official) {
...
if(args[0]==='-p'&&args[1]==='/config') {
 const first=!fs.existsSync(marker);
 if(first) {fs.writeFileSync(marker,'claimed');log({event:'config-stall-start'});}
 const result=spawnSync(${JSON.stringify(official)},args,{encoding:'utf8',maxBuffer:1024*1024});
 if(first&&result.status===0) {log({event:'config-stalled'});setInterval(()=>{},1000);}
 else {log({event:'config-forwarded',code:result.status});process.stdout.write(result.stdout||'');process.stderr.write(result.stderr||'');process.exitCode=result.status??1;}
} else {
