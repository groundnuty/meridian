...
function renderContent() {
  ...
  let html = ''
  if (page === 'Overview') {
    html = `<div class="hero"><div><div class="live-label"><i class="dot ${current.running ? 'healthy' : ''}"></i>${current.running ? 'LIVE CONNECTION' : 'AWAITING CONNECTION'}</div><h2>${current.running ? 'Your local Claude, at a glance.' : 'A home for your Meridian.'}</h2><p>${current.running ? 'Usage, cache health, and every request. Your service and its current owner are always visible.' : 'Connect your existing setup to see its health, limits, and activity in one place.'}</p></div><div class="hero-meta"><span class="eyebrow">SERVICE OWNER</span><strong>${current.owned ? 'Meridian Desktop' : 'External'}</strong><small>${current.owned ? 'Supervised by this app' : 'Docker · Nix · CLI · service manager'}</small></div></div>${stats()}<div class="overview-grid">${section('Cache continuity', 'Reuse across the latest conversations', activity())}${section('Service details', '', `<dl><div><dt>Connection</dt><dd>${current.running ? 'Connected' : 'Offline'}</dd></div><div><dt>Health</dt><dd>${esc(health.status || 'Unavailable')}</dd></div><div><dt>Version</dt><dd class="mono">${esc(current.running || '—')}</dd></div><div><dt>Profiles</dt><dd>${count(rows(object(current.profiles).profiles).length)}</dd></div><div><dt>Management</dt><dd>${current.owned ? 'App managed' : 'Connect only'}</dd></div></dl>`)}</div>${section('Usage limits', 'Headroom and reset times, by account', quotas(2))}${section('Recent requests', 'The latest activity through your connected service', requestTable(5))}`
